...
export const Cards = (props) => {
  ...
  const handleRequest = () => {
    axios
      .post(
        "http://localhost:5000/api/v1/place/request-place",
        {
          owner: props.name,
          placeID: props.pID,
      ...
      .then((res) => {
        console.log(res);
        if (res.data.message.includes("is not") || res.data.error) {
          props.setError(true);
  ...
  var valid = function (current) {
    for (let i = 0; i < props.dates.length; i++) {
      if (
        current < moment(props.dates[i]["checkIn"].split("T")[0]) ||
        current > moment(props.dates[i]["checkOut"].split("T")[0])
      ) {
      } else {
        return false;
      }
